...
Opening `/order/:order_token` in a browser renders a reusable reception-number card with image save/share/copy controls and local previous-order recall. API clients that do not request `text/html` keep the existing JSON response, and `/api/public/orders/:order_token` returns the same order payload with store profile data. Public page actions are recorded in `order_page_events` with a hashed IP and bounded event names.
