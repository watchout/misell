(function () {
  ...
  async function loadEditor(options = {}) {
    ...
    const [response, handoffResponse, scheduleHandoffResponse, cutPlanResponse] = await Promise.all([
  ...
  function statusClass(status) {
    if (status === "valid" || status === "validated" || status === "passed" || status === "active") return "success";
    if (status === "invalid" || status === "deleted" || status === "failed") return "failed";
    return "pending";
  }
