...
- playlistは `data/playlist.json` と `data/playlist.schema.json` で検証する
...
- 商用前にheartbeat、クラウドログ回収、アラート通知を追加する
...
- `scripts/collect-device-evidence.sh`: status、heartbeat、service/timer、journal、ログの証跡回収
