...
- Cloud playlist同期を使う端末は `INSTALL_CONTENT_SYNC=1 scripts/setup-autostart.sh` で `misell-content-sync.timer` を有効化する
- `scripts/sync-content.sh --dry-run` でCloudのplaylist配信指示だけを確認できる
...
- `scripts/backup-content.sh --reason rollout` でコンテンツバックアップが作成されることを確認する
...
- `scripts/backup-content.sh`: playlist、config、assetsの世代バックアップ
- `scripts/sync-content.sh`: Cloud content manifestの取得、playlist反映、結果報告
