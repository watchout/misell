...
Cloud is the source of truth for store settings, offer definitions, QR links, counter orders, and device event idempotency. Local terminal state is execution/cache state and must be able to backfill events.

### Store settings

Store settings are scoped by `store_id`.

- `timezone`
- `business_day_start_time`
- `order_issue_cutoff_time`
- pickup window
- currency
- tax included flag

`order_issue_cutoff_time` is evaluated on the store's business-day timeline, not by a simple same-day clock comparison. If `business_day_start_time` is `05:00` and `order_issue_cutoff_time` is `02:00`, orders issued at `00:30` belong to the previous business day and remain before cutoff; orders issued after `02:00` are blocked for that business day.

### Offer revisions

Offer revisions are immutable after publication. `offers.current_offer_revision_id` points at the active revision. Changing an active offer creates a new revision instead of mutating the published revision.

### QR revision binding

Counter-order QR links should track `offers.current_offer_revision_id` by default. Existing displayed QR codes remain usable after a new active revision is published because the active revision is resolved at scan/order time.

Explicit revision pinning is allowed only when the admin request supplies `offer_revision_id` or `pin_offer_revision`. Pinned QR links keep using that revision and may become non-orderable if the pinned revision is retired or otherwise invalid.

### Playlog idempotency

Device playlogs should send a stable `event_id` generated by the sender. For rollout compatibility, Cloud accepts legacy payloads without `event_id` and derives a deterministic `legacy-*` id from device and playback fields. Idempotency is enforced by `(tenant_id, device_id, event_id)`.
...
- `event_id`: 推奨。senderが生成する安定ID
- `event_type`: 任意。例: `playback`
- `timestamp` / `occurred_at`: 放映発生時刻
- `playlist_version`
- `playlist_item_id`
- `campaign_id`
- `asset_id`
...
- 既存senderのrolloutを壊さないため、`event_id` がないpayloadも受け付ける
- Cloudはdevice/playback fieldsから deterministic な `legacy-*` event idを生成する
- 同じ `(tenant_id, device_id, event_id)` の再送は `duplicate=true` を返し、二重insertしない
