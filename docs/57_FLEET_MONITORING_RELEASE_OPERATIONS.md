...
- クラウド側device_token発行/失効/認証
- クラウド側heartbeat受信
- クラウド監視/通知
- クラウドログ回収
- release manifest
- 自動/半自動rollback
- 端末ごとのrelease_channel管理画面
- Tailscale ACLの正式運用

## 次の実装候補

1. `docs/58_CLOUD_MONITORING_MVP_SPEC.md` に従って `apps/cloud` を実装する
2. Remote CMS側に `POST /api/device/heartbeat` を実装する
3. device_token発行/失効をクラウド管理画面へ実装する
4. release manifest配布を実装する
5. staging/canary/stableを管理画面で切り替えられるようにする
6. 自動rollbackを実装する
