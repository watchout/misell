...
現時点では、Cloudで動画/画像ファイルをアップロード、保管、downloadできる。ただし、content manifestへ必要素材として紐づけ、端末が自動downloadして配置するasset syncはまだない。

そのため、今すぐできる動画入れ替えは次のいずれかになる。

| 方法 | 内容 | 適した場面 |
| --- | --- | --- |
| 既存動画の切り替え | 端末に既にある `/assets/videos/...` をplaylistで指定する | 現地またはLAN管理画面で素材投入済み |
| Cloud保管済み素材の準備 | Cloud素材ライブラリに動画/画像を置き、sha256とdownload pathを管理する | 次回配信前の素材台帳、運用準備 |
| デモ/生成コンテンツの切り替え | `/demo/...` や `/generated/...` をplaylistで指定する | 商談デモ、簡易検証 |

Cloud content manifestは、端末の `release_channel` に一致するactive manifestを返す。端末は `scripts/sync-content.sh` でplaylistを受け取り、検証に通れば `apps/player/data/playlist.json` を差し替える。
...
実装状態: 実装済み。次はStep 3でcontent manifestとの素材紐づけと端末asset syncを進める。
