...
協賛枠はMisell側で固定しない。導入先が自分たちの施設、客層、既存スポンサー、イベント、地域広告の売り方に合わせて自由に構成できるようにする。
...
- 15秒CMだけを協賛枠にする
- 3面ぶち抜き枠を常に最上位商品にする
- 価格、掲載期間、表示回数をMisell標準だけに縛る

導入先ごとに設定できる協賛商品:

| 設定 | 例 |
| --- | --- |
| 商品名 | 地域スポンサー枠、イベント協賛、フロア案内協賛、商品紹介枠、月間ジャック |
| 対象面 | 3面ぶち抜き、左+中央、中央+右、単面、QRパネル、ticker |
| 表示比率 | 全体の5%、10%、20%、時間帯限定 |
| 表示時間 | 5秒、10秒、15秒、30秒、指定素材尺 |
| 期間 | 単日、週、月、イベント期間、任意開始/終了日 |
| 対象店舗/場所 | 全店舗、特定店舗、フロア、入口、レジ前、イベント会場 |
| 時間帯 | 終日、ランチ、夕方、週末、イベント前後 |
| 価格方式 | 月額固定、期間固定、手動見積、インプレッション参考、無償協賛 |
| レポート | 放映回数、表示時間、QR読み取り、画面占有率、改善メモ |
| 承認 | 店舗承認必須、Misell承認必須、広告主確認待ち |

データ上は「slot」ではなく、`sponsorship_product` と `campaign_placement` として扱う。`campaign` は広告主と期間と目的を持ち、`campaign_placement` がどの協賛商品に、どの画面構成で、どの比率/時間帯で入るかを持つ。最終的な `playlist` はこの配置ルールから生成または手動編集する。

必要な追加DB候補:

| table | 役割 |
| --- | --- |
| `sponsorship_products` | 導入先ごとの協賛商品定義。名称、説明、価格メモ、許可layout、標準尺、表示比率、承認要否を持つ |
| `campaign_placements` | campaignを協賛商品、店舗、画面構成、時間帯、優先度へ割り当てる |
| `campaign_assets` | campaignで使う素材と役割。main video、wide background、QR panel、logo、still等を分ける |
| `playlist_rules` | campaign placementからplaylistへ反映する生成ルール。手動playlist運用なら後続でもよい |

PR1の `advertisers`, `campaigns`, `applications`, `qr_links`, `qr_scans`, `report_snapshots` は土台として使える。ただし、導入先が自由に協賛商品を構成する完成形には、上記の追加テーブル/API/UIが必要である。
...
| `GET` | `/api/admin/content-rollouts/:content_id` | 端末別の同期状態 |
| `POST` | `/api/admin/content-rollouts/:content_id/devices/:device_id/retry` | 端末別の素材再同期状態リセット |
